function [out_decoder,out_decoder_test] = conv_lstm_predict(Train_inputs,Train_targets,Test_inputs,Test_targets,layer_number,bias,filters,batch_size,epoch)
eta1 = 0.000000001;
eta2 = 0.000000001;
% kernel
Wxi = filters.Wxi;
...
e = e+immse(Train_targets{channel_count,input_count},out_decoder{channel_number_count,input_count});
end
...
for decoder_channel_number=1:layer_number
    for decoder_kernel_number = 1:channel_number
        ...
        filters.Wdecoder{decoder_channel_number,decoder_kernel_number} = filters.Wdecoder{decoder_channel_number,decoder_kernel_number} + ...
            eta1*conv2(HH,(Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number,input_count}),'valid'); 
    end
end
for decoder_kernel_number = 1:channel_number
   bias.decoder{decoder_kernel_number} = bias.decoder{decoder_kernel_number}+eta1*(Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number});
end
% update decoder
...
Wxi{decoder_kernel_number,decoder_channel_number} = Wxi{decoder_kernel_number,decoder_channel_number} + ...
  eta2*conv2(filters.Wdecoder{decoder_channel_number,decoder_kernel_number},(Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number,input_count})...
...
bi{decoder_channel_number} = bi{decoder_channel_number} + ...
  eta2*conv2((Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number,input_count})...
...
Who{decoder_channel_number} = Who{decoder_channel_number} + ...
  eta2*conv2(filters.Wdecoder{decoder_channel_number,decoder_kernel_number},(Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number,input_count}).*(tanh(ct{decoder_channel_number,input_count})),'same').*...
...
bo{decoder_channel_number} = bo{decoder_channel_number} + ...
  eta2*conv2(((Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number,input_count}).*tanh(ct{decoder_channel_number,input_count})),filters.Wdecoder{decoder_channel_number,decoder_kernel_number},'same').*...
...
Wxf{decoder_kernel_number,decoder_channel_number} = Wxf{decoder_kernel_number,decoder_channel_number} + ...
  (conv2(filters.Wdecoder{decoder_channel_number,decoder_kernel_number},(Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number,input_count}).*(ot{decoder_channel_number,input_count}.*(1-tanh(ct{decoder_channel_number,input_count}))),'same')).*...
...
bf{decoder_channel_number} = bf{decoder_channel_number} + ...
  (conv2((Train_targets{channel_count,input_count}-out_decoder{decoder_kernel_number,input_count}).*(ot{decoder_channel_number,input_count}.*(1-tanh(ct{decoder_channel_number,input_count}))),filters.Wdecoder{decoder_channel_number,decoder_kernel_number},'same')).*...
...
out_decoder_temp = zeros(row_number,column_number);
for decoder_channel_number=1:layer_number
out_decoder_temp = out_decoder_temp + conv2(Ht_test{decoder_channel_number,input_count_test},filters.Wdecoder{decoder_channel_number,decoder_kernel_number},'same');
end
out_decoder_test{decoder_kernel_number,input_count_test} = out_decoder_temp+bias.decoder{decoder_kernel_number};
end

end
for i=1:length(out_decoder)
vv =icwt(out_decoder{i});
vvv(i) = vv(end);
end
for i=1:length(Train_targets)
vv2 =icwt(Train_targets{i});
vvv2(i) = vv2(end);
end
for i=1:length(out_decoder_test)
vv3 =icwt(out_decoder_test{i});
vvv3(i) = vv3(end);
end
for i=1:length(Test_targets)
vv4 =icwt(Test_targets{i});
vvv4(i) = vv4(end);
end
epoch = epoch_count
error_train = mse(vvv,vvv2)
error_test = mse(vvv3,vvv4)
end